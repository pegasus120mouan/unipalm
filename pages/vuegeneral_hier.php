...
$getSommeGlobalQuery = "SELECT SUM(commandes.cout_global) AS sum_cout_global 
                        ...
                        WHERE DATE(commandes.date_commande) = DATE(CURRENT_DATE() - INTERVAL 1 DAY)
...
$getSommeDepenseQuery = "SELECT SUM(depense) AS somme_depense, DATE(date_commande) AS date_commande, livreurs.nom, livreurs.prenoms
FROM points_livreurs
JOIN livreurs ON points_livreurs.utilisateur_id = livreurs.id
WHERE DATE(date_commande) = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
AND livreurs.id = :id_user
GROUP BY DATE(date_commande), livreurs.nom, livreurs.prenoms";
...
//////////////////////////////////////Somme Gain///////////////////////////////////////////////////////////
$getSommeGainQuery = "SELECT SUM(gain_jour) AS gain, DATE(date_commande) AS date_commande, livreurs.nom, livreurs.prenoms 
FROM points_livreurs 
JOIN livreurs ON points_livreurs.utilisateur_id = livreurs.id 
WHERE DATE(date_commande) = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) 
AND livreurs.id = :id_user
GROUP BY DATE(date_commande), livreurs.nom, livreurs.prenoms, points_livreurs.date_commande";
